...
const RegisterStaff = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append(
        "FullNameArabic",
        basicInfo.fullNameAr || basicInfo.fullNameEng,
      ); // Required field
      ...
      if (basicInfo.profilePhoto)
        formData.append("PersonalPhotos", basicInfo.profilePhoto);
      ...
      if (qualDoc?.file)
        formData.append("QualificationCertificate", qualDoc.file);

      const medLicDoc = documents.find((d) => d.id === "4");
      if (medLicDoc?.file)
        formData.append("MedicalPracticeLicense", medLicDoc.file);
    ...
    <div className="flex flex-col h-full bg-slate-50 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-2 text-lg">
          ...
          <span className="text-slate-400">Hospital Staff</span>
        </div>
        ...
        <div className="max-w-[1240px] mx-auto">
          ...
          <div className="mb-8">
            ...
            {step === 3 && (
              <UploadDocumentsForm
                documents={
                  ["Lab Technician", "Radiologist", "Pharmacist"].includes(
                    roleDetails.role,
                  )
                    ? documents.filter(
                        (doc) =>
