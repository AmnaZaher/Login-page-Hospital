...
import { Plus, Edit2, Trash2, Building2, Zap, Search, Loader2 } from 'lucide-react';
import { getClinics, getClinicStats, updateClinic, deleteClinic } from '../../../api/clinics';
...
const ClinicsList: React.FC<ClinicsListProps> = ({ onAddClinic, onEditClinic }) => {
      ...
      <div className="max-w-[1200px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Clinics Management</h1>
            <p className="text-slate-500 mt-1">Oversee all satellite facilities and specialized care centers.</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <tr key={clinic.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-slate-900">{clinic.clinicNameAr || 'N/A'}</div>
                            <div className="text-xs text-slate-500">{clinic.specialization}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-slate-900">{clinic.clinicNameEn || 'N/A'}</div>
                            <div className="text-xs text-slate-500">{clinic.specialization}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <div className={`w-3.5 h-3.5 bg-white rounded-full absolute top-0.5 transition-all ${clinic.isActive ? 'left-[22px]' : 'left-1'}`} />
                          </button>
                          <span className={`text-xs font-bold ${clinic.isActive ? 'text-[#1A6FC4]' : 'text-slate-500'}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3 text-slate-400">
                          <button onClick={() => onEditClinic?.(clinic)} className="hover:text-[#1A6FC4] transition-colors">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(clinic.id)} className="hover:text-red-500 transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
          ...
          {!loading && clinics.length > 0 && (
            <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between text-sm text-slate-500">
              <div>
                Showing {(page - 1) * pageSize + 1} to {Math.min(page * pageSize, clinics.length > 0 ? (page-1)*pageSize + clinics.length : 0)} clinics
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900">{stats?.totalClinics || 0}</div>
              <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Clinics</div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-black text-slate-900">{stats?.activeUnits || 0}</div>
              <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">Active Units</div>
            </div>
          </div>
        </div>
